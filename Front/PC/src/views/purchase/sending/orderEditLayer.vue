        ...
        <!-- <el-col :span="11">
          <el-form-item label="是否紧急发货" prop="isUrgentShipment">
            <el-select v-model="ruleForm.isUrgentShipment" class="m-2" :disabled="!props.layer.showButton"
              ...
              <el-option v-for="item in isUrgentShipmentData" :key="item.key" :label="item.value" :value="item.key">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col> -->
        <!-- <el-col :span="11" :offset="2">
          <el-form-item label="是否有足够库存" prop="isSufficientStock">
            <el-select v-model="ruleForm.isSufficientStock" class="m-2" :disabled="!props.layer.showButton"
              ...
              <el-option v-for="item in isSufficientStockData" :key="item.key" :label="item.value" :value="item.key">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col> -->
...
const formRules = {
  ...
  // goodsNo: [{ required: true, message: '请输入物料编码', trigger: 'blur' }],
  // quantity: [{ required: true, message: '请输入计划发货数量', trigger: 'change' }, { validator: validateQuantity, trigger: 'blur' }],
  // palletsQuantity: [{ required: false, message: '请输入发货托数', trigger: 'change' }, { validator: validatePalletsQuantity, trigger: 'blur' }],
   ...
   detailsData.value = [{"goodsId":"S10000005","goodsNo":"SYK200M","goodsName":"石英矿成品1","goodsClassifyGroup":"FinishedProduct","quantity":5555}]
    ...
    if (props.layer.data) {
      ...
      detailsData.value.forEach(detail => {
        ...
        // ruleForm.value.customerGoodsNo = goodsDetail.customerGoodsNo;
        // ruleForm.value.customerIdentificationCode = goodsDetail.customerIdentificationCode;
        // ruleForm.value.quantity = goodsDetail.quantity;
        // ruleForm.value.palletsQuantity = goodsDetail.palletsQuantity;
...
//   ruleForm.value.customerGoodsNo = goodsArr[0].customerGoodsNo;
//   ruleForm.value.customerIdentificationCode = goodsArr[0].customerIdentificationCode;
//   ruleForm.value.quantity = 0;
//   ruleForm.value.palletsQuantity = 0;
...
const onSelectGoods = (selectedGoods: any) => {
  ...
  detailsData.value.forEach(f => {
    ...
    // if (f.quantity > 0) {
    //   if (f.packageCount > 0 && f.maxPackageCount > 0) {
    //     f.palletsQuantity = Math.ceil(Number((Number(ruleForm.value.quantity) / f.packageCount) / f.maxPackageCount));
    //   }
    // }
  });
}
...
const onInputQuantity = (detail: any) => {
  if (detail.quantity && detail.quantity > 0 && detail.packageCount > 0 && detail.maxPackageCount > 0) {
    detail.palletsQuantity = Math.ceil(Number((Number(detail.quantity) / detail.packageCount) / detail.maxPackageCount));
  }
}
...
const submit = () => {
  formRef.value.validate((valid: any) => {
    ...
    if (valid) {
      ...
      //   customerGoodsNo: ruleForm.value.customerGoodsNo,
      //   customerIdentificationCode: ruleForm.value.customerGoodsNo,
