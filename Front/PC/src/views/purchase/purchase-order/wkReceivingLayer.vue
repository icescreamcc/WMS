<template>
          ...
          <el-tab-pane label="WK收货信息" name="receve">
            <el-row>
                ...
                <el-form-item label="收货状态" prop="receivingStatus">
                  <el-select v-model="ruleForm.receivingStatus" class="m-2" :disabled="!props.layer.showButton"  style="width:100%" placeholder="选择收货状态">     
  ...
  const ruleForm = ref({
      ...
      mnaNo:commonHelper.formatToDate(props.layer.data?.mnaNo),
      ...
      receivingDate:commonHelper.formatToDateTime(props.layer.data?.receivingDate)|| commonHelper.formatToDateTime(new Date()),
  ...
  const validateGR = (rule: any, value: any, callback: any) => {
    if(ruleForm.value.receivingStatus=='Arrived'){
      if(!value){
        callback(new Error('确认已到货时，需要填写GR号')); 
      }  
    } 
      callback()
  }
  ...
  const rules = {  
      grNo: [{  validator: validateGR, trigger: 'blur' },{ max: 50, message: '输入的字符数不能超过50个', trigger: 'blur'}],
      receivingAbnormalReason:[{ max: 200, message: '输入的字符数不能超过200个', trigger: 'blur'}], 
  }  
