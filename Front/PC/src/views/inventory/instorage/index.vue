  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        <el-button v-if="permission.isPermisstion('INSTORAGEORDERADD')" type="primary"
          icon="el-icon-circle-plus-outline" @click="handleAdd">新增</el-button>

        <span style="margin-right: 10px;"></span>
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('INSTORAGEORDERDEL')"
        ...
        <el-popconfirm :title='apprvalMsg'
          v-if="permission.isPermisstion('INSTORAGEORDERAPPROVAL') && isInStorageApproval"
      ...
      <div class="layout-container-form-search">
        <el-select v-model="selectedGoodsGroup" size="small" class="m-2" style="width:100%;margin-right:10px;"
          @change="onClassifyChanged" placeholder="选择物品大类">
          <template #prefix>
            <div
        ...
        <el-button v-if="permission.isPermisstion('INSTORAGEORDEREXPORT')" icon="el-icon-download"
    ...
    <div class="layout-container-table">
      <Table ref="table" v-model:page="page" v-loading="loading" :showSelection="true" :data="tableData"
        ...
        <el-table-column prop="orderNo" label="入库单号" align="center" sortable="custom" min-width="100"
        ...
        <el-table-column prop="warehouseName" label="入库仓库" align="center" sortable="custom" min-width="100"
          :show-overflow-tooltip="true" />
        <el-table-column prop="inStorageType" label="入库类型" align="center" sortable="custom" min-width="100"
          :show-overflow-tooltip="true">
          <template #default="props">
        ...
        <el-table-column prop="instorageDate" label="入库时间" align="center" sortable="custom"
          :show-overflow-tooltip="true">
          <template #default="props">
            <span>{{ commonHelper.formatToDateTime(props.row.instorageDate) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="remark" :label="remarkLabel" align="center" sortable="custom"
          :show-overflow-tooltip="true" />
        <el-table-column prop="status" label="状态" align="center" sortable="custom" :show-overflow-tooltip="true">
          <template #default="props">
            ...
            <div v-else-if="props.row.status == 'WaitInStorage'">
              <el-popconfirm title="是否确认入库？" v-if="permission.isPermisstion('INSTORAGEORDERCONFIRM')"
                @confirm="submitInStorage(props.row)">
                <template #reference>
                  <el-button title="点击确认入库" type="warning"
        ...
        <el-table-column label="运输单签字附件" align="center" min-width="80">
        ...
        <el-table-column label="AGV调度" v-if="permission.isPermisstion('INSTORAGEORDERAGVSCHED')" align="center"
          min-width="80" :show-overflow-tooltip="true">
          <template #default="props">
            <img src="../../../../public/icon-img/icon_agv_online.png" v-if="props.row.status == 'WaitInStorage'"
              title="点击调度AGV自动入库" style="height: 28px;cursor: pointer;position: relative;bottom: -4px;"
              @click="onAGVScheduling(props.row)">
          </template>
        </el-table-column>
        <el-table-column prop="approvalDate" v-if="isInStorageApproval" label="审批时间" align="center" min-width="130"
          sortable="custom" :show-overflow-tooltip="true" />
        <el-table-column :label="$t('message.common.handle')" min-width="150" align="left" fixed="right"
          v-if="permission.isPermisstion('INSTORAGEORDERUPDATE', 'INSTORAGEORDERDEL')">
          <template #default="scope">
            <div v-if="scope.row.status == 'WaitInStorage'">
              <el-button @click="handleEdit(scope.row)"
                v-if="permission.isPermisstion('INSTORAGEORDERUPDATE')">{{ $t("message.common.update") }}</el-button>
              <el-popconfirm v-if="permission.isPermisstion('INSTORAGEORDERDEL')" :title="$t('message.common.delTip')"
      ...
      <AutoOperaionLayer :layer="autoOperationLayer" v-if="autoOperationLayer.show"
        @operationFinished="onAgvTaskFinished" />
    </div>
  </div>
</template>
