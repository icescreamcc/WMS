  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        <el-button v-if="permission.isPermisstion('OUTSTORAGEORDERADD')" type="primary" icon="el-icon-circle-plus-outline" @click="handleAdd">新增</el-button>
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('OUTSTORAGEORDERDEL')" @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger"  icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm> 
         <el-popconfirm :title='apprvalMsg' v-if="permission.isPermisstion('OUTSTORAGEORDERAPPROVAL')&&isOutStorageApproval" @confirm="handleApproval(chooseData)">
          <template #reference>
      ...
      <div class="layout-container-form-search">
        <el-select v-model="selectedGoodsGroup" size="small"  class="m-2" style="width:100%;margin-right:10px;" @change="getTableData(true)" placeholder="选择物品大类">
       ...
       <el-date-picker
        ...
        <el-input 
          v-model="query.input"
          placeholder="请输入关键词进行检索(单号/备注/创建人)"
          size="small"
        ></el-input>
        <el-button
          type="primary"
          ...
          >搜索</el-button>  
         <el-button v-if="permission.isPermisstion('OUTSTORAGEORDEREXPORT')" icon="el-icon-download" style="margin-left:20px"  type="info" @click="exportData">导出</el-button>
      </div>
    </div>
    <div class="layout-container-table">
      ...
      >
        ...
        <el-table-column prop="orderNo" label="出库单号" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/>
        <el-table-column prop="warehouseName" label="出库仓库" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/> 
        <el-table-column prop="outStorageType" label="出库类型" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true">
            <template #default="props">
        ...
        <el-table-column prop="outStorageDate" label="出库时间" align="center" sortable="custom" :show-overflow-tooltip="true">
          <template #default="props">
        ...
        <el-table-column prop="status" label="状态" align="center" sortable="custom" :show-overflow-tooltip="true">
              ...
              <div  v-else-if="props.row.status=='WaitOutStorage'">
                   <el-popconfirm title="是否确认出库？" v-if="permission.isPermisstion('OUTSTORAGEORDERCONFIRM')" @confirm="submitOutStorage(props.row)">
                    <template #reference>
                      <el-button  title="点击确认出库" type="warning" :loading="props.row.loading">{{props.row.statusDesc}}</el-button> 
                    </template>
                  </el-popconfirm> 
        ...
        <el-table-column prop="approvalDate" v-if="isOutStorageApproval" label="审批时间" align="center" min-width="130" sortable="custom" :show-overflow-tooltip="true"/>      
        <el-table-column :label="$t('message.common.handle')" align="left" min-width="120" fixed="right" v-if="permission.isPermisstion('OUTSTORAGEORDERUPDATE','OUTSTORAGEORDERDEL')">
          <template #default="scope"> 
            <div v-if="scope.row.status=='WaitOutStorage'">
              <el-button @click="handleEdit(scope.row)"   v-if="permission.isPermisstion('OUTSTORAGEORDERUPDATE')">{{$t("message.common.update")}}</el-button>
                <el-popconfirm v-if="permission.isPermisstion('OUTSTORAGEORDERDEL')"  :title="$t('message.common.delTip')" @confirm="handleDel([scope.row])" >
                  <template #reference>
