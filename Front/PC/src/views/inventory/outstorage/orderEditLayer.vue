<template>
  <Layer :layer="layer" @confirm="submit">
    <el-form :model="ruleForm" :rules="rules" ref="formRef" label-width="auto" label-position="left"
      ...
      <el-row>
        <el-col :span="11">
          <el-form-item v-if="false" label="出库单号" prop="orderNo">
            <el-input v-model="ruleForm.orderNo" disabled placeholder="系统生成 无需填写"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="11" :offset="2">
          <el-form-item v-if="false" label="出库类型" prop="outStorageType">
            <el-select v-model="ruleForm.outStorageType" :disabled="!props.layer.showButton" class="m-2"
              style="width:100%" placeholder="选择出库类型 *必填">
        ...
        <el-col :span="11">
          <el-form-item label="物品类型" prop="goodsClassify">
            <el-select v-model="ruleForm.goodsClassify" class="m-2" :disabled="!props.layer.showButton"
              style="width:100%" @change="onClassifyChanged" placeholder="请选择需要出库的物品大类 *必填">
      ...
      <el-row>
        <el-col :span="11">
          <el-form-item v-if="false" label="出库仓库" prop="warehouseId">
        ...
        <div class="option-content">
          <el-row class="head">
            ...
            <el-col v-if="props.layer.showButton" :span="3" style="text-align:right">
              <el-button v-if="ruleForm.goodsClassify !== 'RawMaterial'" style="margin-bottom:5px" type="success"
                :disabled="!ruleForm.goodsClassify || !props.layer.showButton" @click="onShowGoodsDrawer">选择{{ invTitle
                }}</el-button>
            </el-col>
          </el-row>
          <el-table :data="detailsData" border>
            ...
            <el-table-column v-if="false" prop="minimumContainer" label="出库库位" align="center" min-width="150"
              :show-overflow-tooltip="true">
              <template #default="detail">
                <el-input v-model="detail.row.minimumContainer" :disabled="!props.layer.showButton"
                  placeholder="点击选择出库货位" readonly
                  :title="`出库${detail.row.type == 'WorkbinCell' ? '料箱' : '货位'}${detail.row.minimumContainer || ''}`">
                  <template #prepend>
...
onMounted(() => {
  getOptions().then((res: any) => {
    ...
    if (oOption) {
      ruleForm.value.outStorageType = oOption.key;
    }
