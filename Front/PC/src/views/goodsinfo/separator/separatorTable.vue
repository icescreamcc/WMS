  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button
          ...
          >搜索</el-button>   
       <el-button type="primary" v-if="permission.isPermisstion('SEPARATORADD')"  icon="el-icon-circle-plus-outline" :disabled="addDisabled" @click="handleAdd" >新增</el-button> 
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('SEPARATORDEL')"   @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger" icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm>
           <el-button icon="el-icon-download" v-if="permission.isPermisstion('SEPARATOREXPORT')"  style="margin-left:20px"  @click="getExportAllowField">导出</el-button>
      </div>
    </div>
    <div class="layout-container-table" style="margin-top:-10px">
      <Table
        ...
        >
         <el-table-column prop="goodsId" label="辅材编码" align="center" sortable="custom" min-width="110" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="辅材分类" align="center" sortable="custom"  min-width="110"  :show-overflow-tooltip="true"/>      
            <el-table-column prop="goodsTypeId" label="辅材类型" align="center" sortable="custom" min-width="110" :show-overflow-tooltip="true">
              <template #default="scope"> 
                   <span>{{scope.row.goodsTypeName}}</span> 
                </template>
           </el-table-column>
        <el-table-column prop="goodsName" label="辅材名称" min-width="110" sortable="custom"  :show-overflow-tooltip="true"/> 
         <el-table-column prop="goodsProperty" label="辅材属性" min-width="110" align="center" sortable="custom" :show-overflow-tooltip="true"/> 
        ...
        <el-table-column v-if="permission.isPermisstion('SEPARATORUPDATE','SEPARATORDEL')" :label="$t('message.common.handle')" align="center"  width="160"> 
          <template #default="scope">
            <el-button v-if="permission.isPermisstion('SEPARATORUPDATE')" @click="handleEdit(scope.row)">{{
              $t("message.common.update")
            }}</el-button>
            <el-popconfirm v-if="permission.isPermisstion('SEPARATORDEL')" 
...
import EditModal from './separatorEditLayer.vue'
import {getSeparatorList,getEnableSpareFields,getSeparatorDetail,addSeparator,updateSeparator,delSeparator,exportSeparator,getAllowField} from '@/api/baseinfo/separator'
...
const getTableData = (init: Boolean) => {
  ...
  getSeparatorList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getSeparatorDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const dataSave=(data:any,actionType:string)=>{  
  editLayer.btnLoading=true;
    ...
    if(actionType=='add'){
      addSeparator(data).then(res=>{
        editLayer.show = false;
        getTableData(true);
      }).finally(()=> editLayer.btnLoading=false);
    }
    else{
        updateSeparator(data).then(res=>{
        editLayer.show = false;
        getTableData(false);
      }).finally(()=> editLayer.btnLoading=false);
    }
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportSeparator(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
