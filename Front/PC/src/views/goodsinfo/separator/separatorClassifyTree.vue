     ...
     <el-row style="width:100%">
       <el-col :span="12" style="text-align:left">
        ...
        <el-col :span="12"  style="text-align:right"> 
           <el-tooltip v-if="permission.isPermisstion('SEPARATORTYPEADD')"  content="新增" placement="bottom" effect="light">
               <el-button type="primary" icon="el-icon-circle-plus-outline"  @click="handleAdd" ></el-button>
          </el-tooltip>
             <el-tooltip v-if="permission.isPermisstion('SEPARATORTYPEUPDATE')" content="编辑" placement="left" effect="light">
               <el-button type="warning" icon="el-icon-edit" :disabled="editDisabled" @click="handleUpdate" ></el-button>
          </el-tooltip>
         <el-popconfirm v-if="permission.isPermisstion('SEPARATORTYPEDEL')" :title="delTitie"  @confirm="handleDel()">
          <template #reference>
...
import {getSeparatorClassifyData,addSeparatorClassify,updateSeparatorClassify,delSeparatorClassify} from '@/api/baseinfo/separator'  
import permission from '@/utils/system/permission'
import Layer from './separatorClassifyEditLayer.vue'  
...
const getTreeData = (selectId:any=null) => {  
  getSeparatorClassifyData().then((res:any)=>{  
...
const dataSave=(data:any,actionType:string)=>{ 
  layer.btnLoading=true;
    if(actionType=='add'){
      addSeparatorClassify(data).then(res=>{
        layer.show = false;
        getTreeData();
      }).finally(()=> layer.btnLoading=false);
    }
    else{
        updateSeparatorClassify(data).then(res=>{
        layer.show = false;
  ...
  const handleDel=()=>{ 
    ...
    delSeparatorClassify(active.value.id)
      .then(()=>{ getTreeData(); })
