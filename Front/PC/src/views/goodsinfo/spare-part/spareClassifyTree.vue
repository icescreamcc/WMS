...
import {getSparePartClassifyData,addSparePartType,updateSparePartType,delSparePartType} from '@/api/baseinfo/sparePart'  
import permission from '@/utils/system/permission'
import Layer from './spareClassifyEditLayer.vue'  
...
const getTreeData = (selectId:any=null) => {  
  getSparePartClassifyData().then((res:any)=>{  
...
const dataSave=(data:any,actionType:string)=>{ 
  layer.btnLoading=true;
    if(actionType=='add'){
      addSparePartType(data).then(res=>{
        layer.show = false;
        getTreeData();
      }).finally(()=> layer.btnLoading=false);
    }
    else{
        updateSparePartType(data).then(res=>{
        layer.show = false;
  ...
  const handleDel=()=>{ 
    ...
    delSparePartType(active.value.id)
      .then(()=>{ getTreeData(); })
