  ...
  <div class="layout-container">
    ...
    <div class="layout-container-table" style="margin-top:-10px">
      <Table
        ...
        >
         <el-table-column prop="goodsId" label="备件编码" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="备件分类" align="center" sortable="custom"  min-width="100"  :show-overflow-tooltip="true"/>      
            <el-table-column prop="goodsTypeId" label="备件类型" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true">
              <template #default="scope"> 
                   <span>{{scope.row.goodsTypeName}}</span> 
                </template>
           </el-table-column>
        <el-table-column prop="goodsName" label="备件名称" min-width="100" sortable="custom"  :show-overflow-tooltip="true"/>
        <el-table-column prop="goodsModel" label="备件型号" min-width="100" sortable="custom" :show-overflow-tooltip="true"/>  
             <el-table-column prop="goodsProperty" label="备件属性" min-width="100" align="center" sortable="custom" :show-overflow-tooltip="true"/> 
...
import EditModal from './spareEditLayer.vue'
import {getSparePartList,getEnableSpareFields,getSparePartDetail,addSparePart,updateSparePart,delSparePart,exportSparePart,getAllowField} from '@/api/baseinfo/sparePart'
...
const getTableData = (init: Boolean) => {
  ...
  getSparePartList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getSparePartDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const dataSave=(data:any,actionType:string)=>{  
  editLayer.btnLoading=true;
    ...
    if(actionType=='add'){
      addSparePart(data).then(res=>{
        editLayer.show = false;
        getTableData(true);
      }).finally(()=> editLayer.btnLoading=false);
    }
    else{
        updateSparePart(data).then(res=>{
        editLayer.show = false;
        getTableData(false);
      }).finally(()=> editLayer.btnLoading=false);
    }
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportSparePart(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
