   ...
   <el-form  label-width="auto" label-position="left" :model="ruleForm" :rules="rules" ref="formRef" style="min-height:400px;margin-top: -10px;">
      <el-tabs v-model="tabSelected" style="box-shadow:none;-webkit-box-shadow:none;min-height:380px">
        <el-tab-pane label="基本信息" name="base">
              <el-row> 
        <el-col :span="11">
        <el-form-item label="备件ID" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsId" readonly placeholder="系统生成 无需填写"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11"  >
       <el-form-item label="备件分类"  style="margin-left:20px">
      ...
      <el-row> 
        ...
        <el-form-item label="SAP编码" prop="goodsNo" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsNo"  placeholder="请输入备件编码" autofocus></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
         <el-form-item label="备件名称" prop="goodsName" style="margin-left:20px" >
        <el-input v-model="ruleForm.goodsName"  placeholder="请输入备件名称  *必填"></el-input>
      </el-form-item> 
        </el-col>
      </el-row> 
        <el-row>
        <el-col :span="11"> 
           <el-form-item label="备件型号" prop="goodsModel" style="margin-left:20px">
             <el-input v-model="ruleForm.goodsModel" placeholder="输入备件的具体型号  *必填"></el-input>
          </el-form-item>  
        </el-col>
        <el-col :span="11" >
           <el-form-item label="备件类型" style="margin-left:20px"> 
             <el-select v-model="ruleForm.goodsTypeId"  class="m-2" placeholder="选择备件类型" style="width:100%" >
                  <el-option
                    v-for="item in sparePartTypeData"
                    :key="item.optionId"
      ...
      <el-row>
        <el-col :span="11"> 
           <el-form-item label="所属区域" prop="forArea" style="margin-left:20px">
            <el-select v-model="ruleForm.forArea"  class="m-2" placeholder="选择所属车间区域  *必填" style="width:100%" >
            <el-option key="All" value="All" label="所有区域"></el-option>
            <el-option
       ...
       <el-row> 
        <el-col :span="11">
        <el-form-item label="备件属性" prop="goodsProperty" style="margin-left:20px"> 
          <el-input v-model="ruleForm.goodsProperty" placeholder="输入备件属性、参数等信息"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
      ...
      <el-row> 
        <el-col :span="11" >
           <el-form-item label="储存规格" style="margin-left:20px" prop="goodsSpecificationId"> 
             <el-select v-model="ruleForm.goodsSpecificationId"   class="m-2" placeholder="选择备件存放料箱的规格" style="width:100%" >
                  <el-option
      ...
      <el-row>
        <el-col :span="11" >
                  <el-form-item label="按此规格入库" style="margin-left:20px;text-align: left;" prop="isConstraintSpec">
                    <el-checkbox disabled label="是" v-model="ruleForm.isConstraintSpec"/>    
               </el-form-item>
              </el-col>
        <el-col :span="11">
       ...
       <el-row>
        <el-col :span="24" style="margin-bottom:40px">
           <el-form-item label="备件图片"  style="height:50px;margin-left:20px">
        ...
        <el-col :span="11" >
          ...
          <el-tab-pane label="其他信息">   
              ...
              <el-form-item label="保质期(月)" style="margin-left:20px" prop="expirationDate">
                <el-input-number v-model="ruleForm.expirationDate" style="width: 100%;" controls-position="right" :min="0"/>
            </el-form-item>
              </el-col>
            </el-row>
               <el-row>
           ...
           <el-row> 
            ...
            <el-row> 
              <el-col :span="11">
              <el-form-item label="不再采购" style="margin-left:20px;text-align: left;" prop="isNotPurchase">
                    <el-checkbox  label="是" v-model="ruleForm.isNotPurchase"/>    
               </el-form-item>
              </el-col>
              <el-col :span="11" >
              <el-form-item label="保修期(天)" style="margin-left:20px" prop="warrantyPeriodDate">
                <el-input-number v-model="ruleForm.warrantyPeriodDate" controls-position="right" style="width: 100%;" :min="0"/>
            </el-form-item>
              </el-col>
            </el-row>
          </el-tab-pane> 
...
import {getOptions} from '@/api/baseinfo/sparePart'
...
const props=defineProps({
 ...
 const ruleForm = ref( {
      ...
      warrantyPeriodDate:props.layer.row?.warrantyPeriodDate,
      ...
      isNotPurchase:props.layer.row?.isNotPurchase,
  ...
  const sparePartTypeData=ref(new Array<any>());
  ...
  onMounted(()=>{
    ...
    if(props.layer.type=='add'){
      ...
      ruleForm.value.isConstraintSpec=true;
    }
    else{
  ...
  const sapNoValid=(rule:any,value:any,callback:any)=>{
     if(value&&!ruleForm.value.goodsNo?.trim()){
      callback(new Error("请输入备件在SAP中的编码"))
     }
     else{
        callback()
     } 
  ...
  const rules :any= {
      ...
      goodsName: [{ required: true, message: '请输入备件名称', trigger: 'blur' },{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],
      goodsModel:[{ required: true, message: '请输入备件型号', trigger: 'blur' },{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],  
      goodsProperty:[{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],  
      forArea:[{ required: true, message: '请选择所属区域', trigger: 'blur' }],
      ...
      goodsSpecificationId:[{validator:specificationValid,trigger: 'blur'}] 
    } 
  
  const getOptionData=()=>{
    getOptions().then(res=>{
    sparePartTypeData.value=res.data.sparePartTypeOptions?.argsOptions;
