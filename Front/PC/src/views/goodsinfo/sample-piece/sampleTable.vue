    ...
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button
          ...
          >搜索</el-button>   
       <el-button type="primary" v-if="permission.isPermisstion('SAMPLEPIECEADD')"  icon="el-icon-circle-plus-outline" :disabled="addDisabled" @click="handleAdd" >新增</el-button> 
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('SAMPLEPIECEDEL')"   @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger" icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm>
           <el-button icon="el-icon-download" v-if="permission.isPermisstion('SAMPLEPIECEEXPORT')"  style="margin-left:20px"  @click="getExportAllowField">导出</el-button>
      </div>
    </div>
      ...
      <Table
        ...
        >
         <el-table-column prop="goodsNo" label="SAP编码" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="样件分类" align="center" sortable="custom"  min-width="100"  :show-overflow-tooltip="true"/>  
        <el-table-column prop="goodsName" label="样件名称" min-width="100" sortable="custom"  :show-overflow-tooltip="true"/>  
        ...
        <el-table-column v-if="permission.isPermisstion('SAMPLEPIECEUPDATE','SAMPLEPIECEDEL')" :label="$t('message.common.handle')" align="center"  width="160"> 
          <template #default="scope">
            <el-button v-if="permission.isPermisstion('SAMPLEPIECEUPDATE')" @click="handleEdit(scope.row)">{{
              $t("message.common.update")
            }}</el-button>
            <el-popconfirm v-if="permission.isPermisstion('SAMPLEPIECEDEL')" 
...
import EditModal from './sampleEditLayer.vue'
import {getSamplePieceList,getEnableSpareFields,getSamplePieceDetail,addSamplePiece,updateSamplePiece,delSamplePiece,exportSamplePiece,getAllowField} from '@/api/baseinfo/sample'
...
const getTableData = (init: Boolean) => {
  ...
  getSamplePieceList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getSamplePieceDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const dataSave=(data:any,actionType:string)=>{  
  editLayer.btnLoading=true;
    ...
    if(actionType=='add'){
      addSamplePiece(data).then(res=>{
        editLayer.show = false;
        getTableData(true);
      }).finally(()=> editLayer.btnLoading=false);
    }
    else{
        updateSamplePiece(data).then(res=>{
        editLayer.show = false;
        getTableData(false);
      }).finally(()=> editLayer.btnLoading=false);
    }
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportSamplePiece(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
