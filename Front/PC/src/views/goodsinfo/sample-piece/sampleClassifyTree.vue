     ...
     <el-row style="width:100%">
       <el-col :span="12" style="text-align:left">
        ...
        <el-col :span="12"  style="text-align:right"> 
           <el-tooltip v-if="permission.isPermisstion('SAMPLEPIECETYPEADD')"  content="新增" placement="bottom" effect="light">
               <el-button type="primary" icon="el-icon-circle-plus-outline"  @click="handleAdd" ></el-button>
          </el-tooltip>
             <el-tooltip v-if="permission.isPermisstion('SAMPLEPIECETYPEUPDATE')" content="编辑" placement="left" effect="light">
               <el-button type="warning" icon="el-icon-edit" :disabled="editDisabled" @click="handleUpdate" ></el-button>
          </el-tooltip>
         <el-popconfirm v-if="permission.isPermisstion('SAMPLEPIECETYPEDEL')" :title="delTitie"  @confirm="handleDel()">
          <template #reference>
...
import {getSamplePieceClassifyData,addSamplePieceClassify,updateSamplePieceClassify,delSamplePieceClassify} from '@/api/baseinfo/sample'  
import permission from '@/utils/system/permission'
import Layer from './sampleClassifyEditLayer.vue'  
...
const getTreeData = (selectId:any=null) => {  
  getSamplePieceClassifyData().then((res:any)=>{  
...
const dataSave=(data:any,actionType:string)=>{ 
  layer.btnLoading=true;
    if(actionType=='add'){
      addSamplePieceClassify(data).then(res=>{
        layer.show = false;
        getTreeData();
      }).finally(()=> layer.btnLoading=false);
    }
    else{
        updateSamplePieceClassify(data).then(res=>{
        layer.show = false;
  ...
  const handleDel=()=>{ 
    ...
    delSamplePieceClassify(active.value.id)
      .then(()=>{ getTreeData(); })
