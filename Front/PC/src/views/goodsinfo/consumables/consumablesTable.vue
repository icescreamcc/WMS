  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button
          ...
          >搜索</el-button>   
       <el-button type="primary" v-if="permission.isPermisstion('SPAREPARTADD')"  icon="el-icon-circle-plus-outline" :disabled="addDisabled" @click="handleAdd" >新增</el-button> 
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('SPAREPARTDEL')"   @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger" icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm>
           <el-button icon="el-icon-download" v-if="permission.isPermisstion('SPAREPARTEXPORT')"  style="margin-left:20px"  @click="getExportAllowField">导出</el-button>
      </div>
    </div>
    <div class="layout-container-table" style="margin-top:-10px">
      <Table
        ...
        >
         <el-table-column prop="goodsId" label="耗材编码" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="耗材分类" align="center" sortable="custom"  min-width="100"  :show-overflow-tooltip="true"/>      
            <el-table-column prop="goodsTypeId" label="耗材类型" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true">
              <template #default="scope"> 
                   <span>{{scope.row.goodsTypeName}}</span> 
                </template>
           </el-table-column>
        <el-table-column prop="goodsName" label="耗材名称" min-width="100" sortable="custom"  :show-overflow-tooltip="true"/>
        <el-table-column prop="goodsModel" label="耗材型号" min-width="100" sortable="custom" :show-overflow-tooltip="true"/>  
             <el-table-column prop="goodsProperty" label="耗材属性" min-width="100" align="center" sortable="custom" :show-overflow-tooltip="true"/> 
        <el-table-column prop="packageCount" label="每包装数量" align="center"  min-width="120" sortable="custom"  :show-overflow-tooltip="true">
           ...
           <el-table-column prop="forArea" label="所属区域" align="center" min-width="120"  sortable="custom" :show-overflow-tooltip="true"/>  
        ...
        <!-- <el-table-column prop="long" label="尺寸(长*宽*高)" align="center" sortable="custom" min-width="130" :show-overflow-tooltip="true">
             <template #default="scope"> 
                   <span v-if="scope.row.long>0||scope.row.wide>0||scope.row.height>0">{{scope.row.long+'*'+scope.row.wide+'*'+scope.row.height+(scope.row.sizeUnitName?scope.row.sizeUnitName:'')}}</span>
                </template>
        </el-table-column>  -->
        <el-table-column prop="costPrice" label="成本价" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true">
            <template #default="scope"> 
                   <span v-if="scope.row.costPrice>0">{{scope.row.costPrice+(scope.row.priceUnitName?scope.row.priceUnitName:'')}}</span>
                </template>
        </el-table-column>    
        ...
        <el-table-column v-if="permission.isPermisstion('SPAREPARTUPDATE','SPAREPARTDEL')" :label="$t('message.common.handle')" align="center"  width="160"> 
          <template #default="scope">
            <el-button v-if="permission.isPermisstion('SPAREPARTUPDATE')" @click="handleEdit(scope.row)">{{
              $t("message.common.update")
            }}</el-button>
            <el-popconfirm v-if="permission.isPermisstion('SPAREPARTDEL')" 
...
import EditModal from './consumablesEditLayer.vue'
import {getConsumablesList,getEnableSpareFields,getConsumablesDetail,addConsumables,updateConsumables,delConsumables,exportConsumables,getAllowField} from '@/api/baseinfo/consumables'
...
const getTableData = (init: Boolean) => {
  ...
  getConsumablesList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getConsumablesDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const dataSave=(data:any,actionType:string)=>{  
  editLayer.btnLoading=true;
    ...
    if(actionType=='add'){
      addConsumables(data).then(res=>{
        editLayer.show = false;
        getTableData(true);
      }).finally(()=> editLayer.btnLoading=false);
    }
    else{
        updateConsumables(data).then(res=>{
        editLayer.show = false;
        getTableData(false);
      }).finally(()=> editLayer.btnLoading=false);
    }
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportConsumables(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
