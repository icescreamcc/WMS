    ...
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button
          ...
          >搜索</el-button>   
       <el-button type="primary" v-if="permission.isPermisstion('FINISHEDPRODUCTADD')"  icon="el-icon-circle-plus-outline" :disabled="addDisabled" @click="handleAdd" >新增</el-button> 
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('FINISHEDPRODUCTDEL')"   @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger" icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm>
           <el-button icon="el-icon-download" v-if="permission.isPermisstion('FINISHEDPRODUCTEXPORT')"  style="margin-left:20px"  @click="getExportAllowField">导出</el-button>
      </div>
    </div>
    <div class="layout-container-table" style="margin-top:-10px">
      <Table
        ...
        <el-table-column prop="goodsNo" label="SAP编码" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="成品分类" align="center" sortable="custom"  min-width="100"  :show-overflow-tooltip="true"/>      
            <!-- <el-table-column prop="goodsTypeId" label="成品类型" align="center" sortable="custom" min-width="100" :show-overflow-tooltip="true">
              <template #default="scope"> 
                   <span>{{scope.row.goodsTypeName}}</span> 
                </template>
           </el-table-column> -->
        <el-table-column prop="goodsName" label="成品名称" min-width="100" sortable="custom"  :show-overflow-tooltip="true"/> 
         <el-table-column prop="customerGoodsNo" label="客户料号" min-width="100" align="center" sortable="custom" :show-overflow-tooltip="true"/>
         <el-table-column prop="customerIdentificationCode" label="客户配送中心" min-width="120" align="center" sortable="custom" :show-overflow-tooltip="true"/> 
        ...
        <el-table-column v-if="permission.isPermisstion('FINISHEDPRODUCTUPDATE','FINISHEDPRODUCTDEL')" :label="$t('message.common.handle')" align="center"  width="250"> 
          <template #default="scope">
            <el-button type="primary" v-if="permission.isPermisstion('FINISHEDPRODUCTBOM')" @click="onShowBOMModal(scope.row)">BOM维护</el-button>
            <el-button v-if="permission.isPermisstion('FINISHEDPRODUCTUPDATE')" @click="handleEdit(scope.row)">{{
              $t("message.common.update")
            }}</el-button>
            <el-popconfirm v-if="permission.isPermisstion('FINISHEDPRODUCTDEL')" 
        ...
        <BomEditLayer :layer="bomtLayer" @dataSubmit="onSubmitBOMData" v-if="bomtLayer.show"/>
    </div>
  </div>
</template>
...
import EditModal from './productEditLayer.vue'
import BomEditLayer from "./bomEditLayer.vue";
import {getFinishedProductList,getEnableSpareFields,getFinishedProductDetail,addFinishedProduct,updateFinishedProduct,delFinishedProduct,exportFinishedProduct,getAllowField,getBOMList,updateBOM} from '@/api/baseinfo/finishedProduct'
...
const bomtLayer: LayerInterface = reactive({
  show: false,
  title: "",
  showButton: true,
  btnLoading:false,
...
const getTableData = (init: Boolean) => {
  ...
  getFinishedProductList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getFinishedProductDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const handleDel = (data: object[]) => {
  ...
  delFinishedProduct(ids).then((res) => { 
...
const dataSave=(data:any,actionType:string)=>{  
  editLayer.btnLoading=true;
    ...
    if(actionType=='add'){
      addFinishedProduct(data).then(res=>{
        editLayer.show = false;
        getTableData(true);
      }).finally(()=> editLayer.btnLoading=false);
    }
    else{
        updateFinishedProduct(data).then(res=>{
        editLayer.show = false;
        getTableData(false);
      }).finally(()=> editLayer.btnLoading=false);
    }
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportFinishedProduct(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
...
const onShowBOMModal=(row:any)=>{
  getBOMList(row.goodsId).then(res=>{ 
    row.bomInfo=res.data; 
    bomtLayer.data=row;
    bomtLayer.show=true;
  })
...
const onSubmitBOMData=(bom:Array<any>)=>{
  bomtLayer.btnLoading=true;
  updateBOM(bom).then(()=>bomtLayer.show=false).finally(()=> bomtLayer.btnLoading=false)
}
</script>
