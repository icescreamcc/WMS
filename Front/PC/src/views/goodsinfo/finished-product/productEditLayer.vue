  ...
  <Layer :layer="layer" @confirm="submit" > 
   <el-form  label-width="auto" label-position="left" :model="ruleForm" :rules="rules" ref="formRef" style="min-height:400px;margin-top: -10px;">
      <el-tabs v-model="tabSelected" style="box-shadow:none;-webkit-box-shadow:none;min-height:380px">
        ...
        <el-row> 
        <el-col :span="11">
        <el-form-item label="成品ID" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsId" readonly placeholder="系统生成 无需填写"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11"  >
       <el-form-item label="成品分类"  style="margin-left:20px">
      ...
      <el-row> 
        ...
        <el-form-item label="SAP编码" prop="goodsNo" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsNo"  placeholder="请输入成品编码" autofocus></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
         <el-form-item label="成品名称" prop="goodsName" style="margin-left:20px" >
        <el-input v-model="ruleForm.goodsName"  placeholder="请输入成品名称  *必填"></el-input>
      </el-form-item> 
        </el-col>
      </el-row> 
        <el-row>
        <el-col :span="11"> 
          <!-- <el-form-item label="成品属性" prop="goodsProperty" style="margin-left:20px"> 
          <el-input v-model="ruleForm.goodsProperty" placeholder="输入成品属性、参数等信息"></el-input>
          </el-form-item>  -->
          <el-form-item label="客户料号" prop="customerGoodsNo" style="margin-left:20px"> 
          <el-input v-model="ruleForm.customerGoodsNo" placeholder="请输入客户料号"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
          <el-form-item label="客户配送中心" prop="customerIdentificationCode" style="margin-left:20px"> 
          <el-input v-model="ruleForm.customerIdentificationCode" placeholder="请输入客户配送中心"></el-input>
          </el-form-item> 

           <!-- <el-form-item label="成品类型" style="margin-left:20px"> 
             <el-select v-model="ruleForm.goodsTypeId"  class="m-2" placeholder="选择成品类型" style="width:100%" >
                  <el-option
                    v-for="item in samplePieceTypeData"
                    :key="item.optionId"
      ...
      <el-row> 
                <el-col :span="11" >
                  <el-form-item label="是否在SAP" style="margin-left:20px;text-align: left;" prop="isInSAP">
                    <el-checkbox  label="是" disabled v-model="ruleForm.isInSAP"/>    
               </el-form-item>
              </el-col>
            </el-row> 
        </el-tab-pane>
      ...
      <el-row> 
        <el-col :span="11" >
           <el-form-item label="储存规格" style="margin-left:20px"> 
             <el-select v-model="ruleForm.goodsSpecificationId"  class="m-2" placeholder="选择成品存放料箱的规格" style="width:100%" >
                  <el-option
      ...
      <el-row>
       ...
       <el-row>
        <el-col :span="24" style="margin-bottom:40px">
           <el-form-item label="成品图片"  style="height:50px;margin-left:20px">
...
import {getOptions} from '@/api/baseinfo/finishedProduct'
...
const props=defineProps({
  ...
  const ruleForm = ref( {
        ...
        customerGoodsNo:props.layer.row?.customerGoodsNo,
        customerIdentificationCode:props.layer.row?.customerIdentificationCode,
  ...
  const samplePieceTypeData=ref(new Array<any>());
  ...
  const sapNoValid=(rule:any,value:any,callback:any)=>{
     if(value&&!ruleForm.value.goodsNo?.trim()){
      callback(new Error("请输入成品在SAP中的编码"))
     }
     else{
        callback()
     } 
  ...
  const rules :any= {
      goodsNo: [{ required: true, message: '请输入成品编码', trigger: 'blur' },{ max: 50, message: '字符超出限制长度', trigger: 'blur'}], 
      goodsName: [{ required: true, message: '请输入成品名称', trigger: 'blur' },{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],
      goodsModel:[{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],  
      // goodsProperty:[{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],  
      customerGoodsNo :[{ max: 50, message: '字符超出限制长度', trigger: 'blur'}], 
      customerIdentificationCode :[{ max: 200, message: '字符超出限制长度', trigger: 'blur'}],
...
const getOptionData=()=>{
  getOptions().then(res=>{
    samplePieceTypeData.value=res.data.samplePieceTypeOptions?.argsOptions;
