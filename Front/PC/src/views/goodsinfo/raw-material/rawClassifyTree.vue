<template>
  <div class="category">
     ...
     <el-row style="width:100%">
       <el-col :span="12" style="text-align:left">
        ...
        <el-col :span="12"  style="text-align:right"> 
           <el-tooltip v-if="permission.isPermisstion('RAWMATERIALTYPEADD')"  content="新增" placement="bottom" effect="light">
               <el-button type="primary" icon="el-icon-circle-plus-outline"  @click="handleAdd" ></el-button>
          </el-tooltip>
             <el-tooltip v-if="permission.isPermisstion('RAWMATERIALTYPEUPDATE')" content="编辑" placement="left" effect="light">
               <el-button type="warning" icon="el-icon-edit" :disabled="editDisabled" @click="handleUpdate" ></el-button>
          </el-tooltip>
         <el-popconfirm v-if="permission.isPermisstion('RAWMATERIALTYPEDEL')" :title="delTitie"  @confirm="handleDel()">
          <template #reference>
   ...
   <RawLayer :layer="layer" v-if="layer.show" @submit="dataSave" />
</template>

<script lang="ts" setup> 
import { LayerInterface } from "@/components/layer/index.vue"; 
import {  ref, reactive,inject, nextTick, Ref,onMounted } from "vue"; 
import {getRawMaterialClassifyData,addRawMaterialClassify,updateRawMaterialClassify,delRawMaterialClassify} from '@/api/baseinfo/rawMaterial'  
import permission from '@/utils/system/permission'
import RawLayer from './rawClassifyEditLayer.vue'  
...
const getTreeData = (selectId:any=null) => {  
  getRawMaterialClassifyData().then((res:any)=>{  
...
const dataSave=(data:any,actionType:string)=>{ 
  layer.btnLoading=true;
    if(actionType=='add'){
      addRawMaterialClassify(data).then(res=>{
        layer.show = false;
        getTreeData();
      }).finally(()=> layer.btnLoading=false);
    }
    else{
        updateRawMaterialClassify(data).then(res=>{
        layer.show = false;
  ...
  const handleDel=()=>{ 
    ...
    delRawMaterialClassify(active.value.id)
      .then(()=>{ getTreeData(); })
