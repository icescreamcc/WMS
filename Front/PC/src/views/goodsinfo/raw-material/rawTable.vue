  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button
          ...
          >搜索</el-button>   
       <el-button type="primary" v-if="permission.isPermisstion('RAWMATERIALADD')"  icon="el-icon-circle-plus-outline" :disabled="addDisabled" @click="handleAdd" >新增</el-button> 
        <el-popconfirm title='确定删除选中的数据吗' v-if="permission.isPermisstion('RAWMATERIALDEL')"   @confirm="handleDel(chooseData)">
          <template #reference>
            <el-button type="danger" icon="el-icon-delete" :disabled="chooseData.length === 0">批量删除</el-button>
          </template>
        </el-popconfirm>
           <el-button icon="el-icon-download" v-if="permission.isPermisstion('RAWMATERIALEXPORT')"  style="margin-left:20px"  @click="getExportAllowField">导出</el-button>
      </div>
    </div>
    <div class="layout-container-table" style="margin-top:-10px">
      <Table
        ...
        >
         <el-table-column prop="goodsNo" label="SAP编码" align="center" sortable="custom" min-width="115" :show-overflow-tooltip="true"/>  
            <el-table-column prop="goodsClassifyName" label="原材料分类" align="center" sortable="custom"  min-width="115"  :show-overflow-tooltip="true"/>    
        <el-table-column prop="goodsName" label="原材料名称" min-width="115" sortable="custom"  :show-overflow-tooltip="true"/> 
         <el-table-column prop="goodsProperty" label="原材料属性" min-width="115" align="center" sortable="custom" :show-overflow-tooltip="true"/> 
        ...
        <el-table-column v-if="permission.isPermisstion('RAWMATERIALUPDATE','RAWMATERIALDEL')" :label="$t('message.common.handle')" align="center"  width="160"> 
          <template #default="scope">
            <el-button v-if="permission.isPermisstion('RAWMATERIALUPDATE')" @click="handleEdit(scope.row)">{{
              $t("message.common.update")
            }}</el-button>
            <el-popconfirm v-if="permission.isPermisstion('RAWMATERIALDEL')" 
...
import EditModal from './rawEditLayer.vue'
import {getRawMaterialList,getEnableSpareFields,getRawMaterialDetail,addRawMaterial,updateRawMaterial,delRawMaterial,exportRawMaterial,getAllowField} from '@/api/baseinfo/rawMaterial'
...
const getTableData = (init: Boolean) => {
  ...
  getRawMaterialList(permission.getOperator().userId, page.size as number,page.index as number,page.orderField,page.orderType,typeid,query.input)
    .then((res) => { 
...
const handleEdit = (row: any) => {  
  getRawMaterialDetail(row.goodsId).then((res:any)=>{
    editLayer.title = "";
...
const exportData=(selField:Array<any>)=>{ 
  ...
  exportRawMaterial(query.input,page.orderField,page.orderType,classifyId,selField).then(res=>{ 
