  ...
  <Layer :layer="layer" @confirm="submit" > 
   <el-form  label-width="auto" label-position="left" :model="ruleForm" :rules="rules" ref="formRef" style="min-height:400px;margin-top: -10px;">
      <el-tabs v-model="tabSelected" style="box-shadow:none;-webkit-box-shadow:none;min-height:380px">
        ...
        <el-row> 
        <el-col :span="11">
        <el-form-item label="包材ID" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsId" readonly placeholder="系统生成 无需填写"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11"  >
       <el-form-item label="包材分类"  style="margin-left:20px">
      ...
      <el-row> 
        ...
        <el-form-item label="SAP编码" prop="goodsNo" style="margin-left:20px">
          <el-input v-model="ruleForm.goodsNo"  placeholder="请输入包材编码" autofocus></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
         <el-form-item label="包材名称" prop="goodsName" style="margin-left:20px" >
        <el-input v-model="ruleForm.goodsName"  placeholder="请输入包材名称  *必填"></el-input>
      </el-form-item> 
        </el-col>
      </el-row> 
        <el-row>
        <el-col :span="11"> 
          <el-form-item label="包材属性" prop="goodsProperty" style="margin-left:20px"> 
          <el-input v-model="ruleForm.goodsProperty" placeholder="输入包材属性、参数等信息"></el-input>
          </el-form-item> 
        </el-col>
        <el-col :span="11" >
           <el-form-item label="包材类型" style="margin-left:20px"> 
             <el-select v-model="ruleForm.goodsTypeId"  class="m-2" placeholder="选择包材类型" style="width:100%" >
                  <el-option
                    v-for="item in packingMaterialTypeData"
                    :key="item.optionId"
      ...
      <el-row> 
        <el-col :span="11" >
           <el-form-item label="储存规格" style="margin-left:20px"> 
             <el-select v-model="ruleForm.goodsSpecificationId"  class="m-2" placeholder="选择包材存放料箱的规格" style="width:100%" >
                  <el-option
      ...
      <el-row>
       ...
       <el-row>
        <el-col :span="24" style="margin-bottom:40px">
           <el-form-item label="包材图片"  style="height:50px;margin-left:20px">
...
import {getOptions} from '@/api/baseinfo/packingMaterial'
...
const props=defineProps({
  ...
  const packingMaterialTypeData=ref(new Array<any>());
  ...
  const sapNoValid=(rule:any,value:any,callback:any)=>{
     if(value&&!ruleForm.value.goodsNo?.trim()){
      callback(new Error("请输入包材在SAP中的编码"))
     }
     else{
        callback()
     } 
  ...
  const rules :any= {
      ...
      goodsName: [{ required: true, message: '请输入包材名称', trigger: 'blur' },{ max: 50, message: '字符超出限制长度', trigger: 'blur'}],
...
const getOptionData=()=>{
  getOptions().then(res=>{
    packingMaterialTypeData.value=res.data.packingMaterialTypeOptions?.argsOptions;
