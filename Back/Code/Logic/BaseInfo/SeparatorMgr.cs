...
namespace Logic.BaseInfo
{
    ...
    /// 辅材管理
    /// </summary>
    public class SeparatorMgr: DataPermissionHandler
    {
        ...
        public SeparatorMgr(Repository repository, IFileStorage fileStorage, IMapper mapper, IConfiguration configuration) : base(repository)
        {
        ...
        {
            var typeData = await Repository.ClientDb.Queryable<BaseType>().Where(w => w.Group == BaseTypeGroup.Separator.ToString()).ToListAsync();
        ...
        public async Task AddSeparatorClassify(BaseTypeDto data)
        {
            ...
            if (existModel)
            {
                throw new BusinessException("保存失败,当前辅材类型编码已存在");
            }
            var model = new BaseType
            {
                ...
                Remark = data.Remark,
                Group = BaseTypeGroup.Separator.ToString()
            };
        ...
        public async Task UpdateSeparatorClassify(BaseTypeDto data)
        {
            ...
            if (!existModel)
            {
                throw new BusinessException("保存失败,当前辅材类型ID不存在或已删除");
            }
            var model = new BaseType
            {
                TypeId = data.TypeId,
        ...
        /// <returns></returns>
        public async Task DelSeparatorClassify(int goodsTypeId)
        {
        ...
        public async Task<TableModel<SeparatorDto>> GetSeparatorList(string userId, int pgSize, int pgIndex, string orderFiled, string orderType, int typeId, string searchKey)
        {
            ...
            var data = Repository.ClientDb.Queryable<BaseGoods>()
                  .LeftJoin<BaseType>((g, a2) => a2.TypeId == g.GoodsClassifyId && a2.Group == BaseTypeGroup.Separator.ToString())
                  .LeftJoin<SysArgsOptions>((g, a2, a3) => a3.OptionId == g.GoodsTypeId && a3.ArgsKey == BusinessConst.SeparatorType)
                  .WhereIF(typeId > 0, (g, a2) => g.GoodsClassifyId == typeId)
                  .WhereIF(typeId == 0, (g, a2) => a2.Group == BaseTypeGroup.Separator.ToString())
                  ...
                  .Select((g, a2 ,a3) => new SeparatorDto
                  {
                      GoodsId = g.GoodsId,
                      GoodsNo = g.GoodsNo,
            ...
            var res = new TableModel<SeparatorDto>() { Total = total, Rows = data };
        ...
        public async Task<SeparatorDto> GetSeparatorDetail(string sampleId)
        {
            var data = await Repository.ClientDb.Queryable<BaseGoods>()
                .LeftJoin<SysArgsOptions>((g, a) => a.OptionId == g.GoodsTypeId && a.ArgsKey == BusinessConst.SeparatorType)
                .LeftJoin<BaseType>((g, a, a2) => a2.TypeId == g.GoodsClassifyId && a2.Group == BaseTypeGroup.Separator.ToString())
                .LeftJoin<InvWorkbinSpecification>((g, a, a2, wbs) => wbs.SpecId == g.GoodsSpecificationId)
                .Where((g, a, a2, wbs) => g.GoodsId == sampleId)
                .Select((g, a, a2, wbs) => new SeparatorDto
                {
                    GoodsId = g.GoodsId,
                    GoodsNo = g.GoodsNo,
                    GoodsName = g.GoodsName,
        ...
        public async Task AddSeparator(SeparatorDto data)
        {
            ...
            if (!string.IsNullOrEmpty(data.GoodsNo))
            {
                ...
                if (exist)
                {
                    throw new BusinessException("保存失败,当前存在相同的辅材名称和型号");
                }
                data.IsInSAP = true;
            }
        ...
        public async Task UpdateSeparator(SeparatorDto data)
        {
            ...
            if (!existId)
            {
                throw new BusinessException("保存失败,当前辅材不存在或已删除");
            }
        ...
        public List<KeyValueModel> GetExportField()
        {
            return new List<KeyValueModel>
            {
                ...
                new KeyValueModel {Key="GoodsName",Value="辅材名称",Remark=1},
                new KeyValueModel {Key="TypeName",Value="辅材分类", Remark = 3},
                new KeyValueModel {Key="OptionName",Value="辅材类型",Remark=4},
                new KeyValueModel {Key="GoodsModel",Value="辅材型号", Remark = 5},
                ...
                new KeyValueModel {Key="Photo",Value="辅材图片", Remark = 28},
        ...
        public async Task<string> ExportSeparator(string searchKey, string orderField, string orderType, int goodsClassifyId, List<KeyValueModel> fields)
        {
            if (fields.Count > 0)
            {
                ...
                string sql = $@"select  {fieldStr}
                            ...
                            where g.IsDeleted=0 and ( case when {goodsClassifyId}=0 then t.Group='Separator' else g.GoodsClassifyId={goodsClassifyId} end ) 
                ...
                var fileName = $"辅材信息导出{DateTime.Now.ToStringNoSignExtension()}.xlsx";
