...
{
    public class SparePartMgr : DataPermissionHandler
    {
        ...
        public SparePartMgr(Repository repository, IFileStorage fileStorage, IMapper mapper, IConfiguration configuration) : base(repository)
        {
        ...
        {
            var typeData = await Repository.ClientDb.Queryable<BaseType>().Where(w => w.Group == BaseTypeGroup.SparePart.ToString()).ToListAsync();
        ...
        public async Task AddSparePartType(BaseTypeDto data)
        {
            ...
            if (existModel)
            {
                throw new BusinessException("保存失败,当前备件类型编码已存在");
            }
            var model = new BaseType
            {
                ...
                Remark = data.Remark,
                Group = BaseTypeGroup.SparePart.ToString()
            };
        ...
        public async Task UpdateSparePartType(BaseTypeDto data)
        {
            ...
            if (!existModel)
            {
                throw new BusinessException("保存失败,当前备件类型ID不存在或已删除");
            }
            var model = new BaseType
            {
                TypeId = data.TypeId,
        ...
        /// <returns></returns>
        public async Task DelSparePartType(int SparePartTypeId)
        {
            await Repository.DeleteAsync<BaseType>(x => x.TypeId == SparePartTypeId);
        }
        ...
        public async Task<TableModel<SparePartDto>> GetSparePartList(string userId, int pgSize, int pgIndex, string orderFiled, string orderType, int goodsClassifyId, string searchKey)
        {
            ...
            var data = Repository.ClientDb.Queryable<BaseGoods>() 
                  ...
                  .WhereIF(goodsClassifyId == 0, (g,a, a2) => a2.Group == BaseTypeGroup.SparePart.ToString())
                  ...
                  .Select((g, a, a2) => new SparePartDto
                  {
                      GoodsId = g.GoodsId,
                      GoodsNo = g.GoodsNo,
            ...
            var res = new TableModel<SparePartDto>() { Total = total, Rows = data };
        ...
        public async Task<SparePartDto> GetSparePartDetail(string spareId)
        {
            var data = await Repository.ClientDb.Queryable<BaseGoods>()
                ...
                .Select((g, a, a2, wbs) => new SparePartDto
                {
                    GoodsId = g.GoodsId,
                    GoodsNo = g.GoodsNo,
                    GoodsName = g.GoodsName,
        ...
        public async Task AddSparePart(SparePartDto data)
        {
            ...
            if (!string.IsNullOrEmpty(data.GoodsNo))
            {
                ...
                if (exist)
                {
                    throw new BusinessException("保存失败,当前存在相同的备件名称和型号");
                }
                data.IsInSAP = true;
            } 
        ...
        public async Task UpdateSparePart(SparePartDto data)
        {
            ...
            if (!existId)
            {
                throw new BusinessException("保存失败,当前备件不存在或已删除");
            }
        ...
        public List<KeyValueModel> GetExportField()
        {
            return new List<KeyValueModel>
            {
                ...
                new KeyValueModel {Key="GoodsName",Value="备件名称",Remark=1},
                new KeyValueModel {Key="TypeName",Value="备件分类", Remark = 3},
                new KeyValueModel {Key="OptionName",Value="备件类型",Remark=4},
                new KeyValueModel {Key="GoodsModel",Value="备件型号", Remark = 5},
                ...
                new KeyValueModel {Key="WarrantyPeriodDate",Value="保修期（天）", Remark = 13},
                ...
                new KeyValueModel {Key="Photo",Value="备件图片", Remark = 28},
        ...
        public async Task<string> ExportSparePart(string searchKey, string orderField, string orderType, int goodsClassifyId, List<KeyValueModel> fields)
        {
            if (fields.Count > 0)
            {
                ...
                string sql = $@"select  {fieldStr}
                            ...
                            where g.IsDeleted=0 and ( case when {goodsClassifyId}=0 then t.Group='SparePart' else g.GoodsClassifyId={goodsClassifyId} end ) 
                ...
                var fileName = $"备件信息导出{DateTime.Now.ToStringNoSignExtension()}.xlsx";
