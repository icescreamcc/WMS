...
{
    public class RawMaterialMgr : DataPermissionHandler
    {
        ...
        public RawMaterialMgr(Repository repository, IFileStorage fileStorage, IMapper mapper, IConfiguration configuration) : base(repository)
        {
        ...
        {
            var typeData = await Repository.ClientDb.Queryable<BaseType>().Where(w => w.Group == BaseTypeGroup.RawMaterial.ToString()).ToListAsync();
        ...
        public async Task AddRawMaterialClassify(BaseTypeDto data)
        {
            ...
            if (existModel)
            {
                throw new BusinessException("保存失败,当前原材料类型编码已存在");
            }
            var model = new BaseType
            {
                ...
                Remark = data.Remark,
                Group = BaseTypeGroup.RawMaterial.ToString()
            };
        ...
        public async Task UpdateRawMaterialClassify(BaseTypeDto data)
        {
            ...
            if (!existModel)
            {
                throw new BusinessException("保存失败,当前原材料类型ID不存在或已删除");
            }
            var model = new BaseType
            {
                TypeId = data.TypeId,
        ...
        /// <returns></returns>
        public async Task DelRawMaterialClassify(int goodsTypeId)
        {
        ...
        public async Task<TableModel<RawMaterialDto>> GetRawMaterialList(string userId, int pgSize, int pgIndex, string orderFiled, string orderType, int typeId, string searchKey)
        {
            ...
            var data = Repository.ClientDb.Queryable<BaseGoods>()
                  .LeftJoin<BaseType>((g, a2) => a2.TypeId == g.GoodsClassifyId && a2.Group == BaseTypeGroup.RawMaterial.ToString())
                  .WhereIF(typeId > 0, (g, a2) => g.GoodsClassifyId == typeId)
                  .WhereIF(typeId == 0, (g, a2) => a2.Group == BaseTypeGroup.RawMaterial.ToString())
                  ...
                  .Select((g, a2) => new RawMaterialDto
                  {
                      GoodsId = g.GoodsId,
                      GoodsNo = g.GoodsNo,
            ...
            var res = new TableModel<RawMaterialDto>() { Total = total, Rows = data };
        ...
        public async Task<RawMaterialDto> GetRawMaterialDetail(string sampleId)
        {
            var data = await Repository.ClientDb.Queryable<BaseGoods>()
                .LeftJoin<SysArgsOptions>((g, a) => a.OptionId == g.GoodsTypeId && a.ArgsKey == BusinessConst.RawMaterialType)
                .LeftJoin<BaseType>((g, a, a2) => a2.TypeId == g.GoodsClassifyId && a2.Group == BaseTypeGroup.RawMaterial.ToString())
                .LeftJoin<InvWorkbinSpecification>((g, a, a2, wbs) => wbs.SpecId == g.GoodsSpecificationId)
                .Where((g, a, a2, wbs) => g.GoodsId == sampleId)
                .Select((g, a, a2, wbs) => new RawMaterialDto
                {
                    GoodsId = g.GoodsId,
                    GoodsNo = g.GoodsNo,
                    GoodsName = g.GoodsName,
        ...
        public async Task AddRawMaterial(RawMaterialDto data)
        {
            ...
            {
                var exist = await Repository.Exist<BaseGoods>(x => x.GoodsNo == data.GoodsNo && x.GoodsName == data.GoodsName && x.Supplier == data.Supplier);
                if (exist)
                {
                    throw new BusinessException("保存失败,当前存在相同的原材料名称、SAP料号和供应商");
                }
                data.IsInSAP = true;
            }
        ...
        public async Task UpdateRawMaterial(RawMaterialDto data)
        {
            ...
            if (!existId)
            {
                throw new BusinessException("保存失败,当前原材料不存在或已删除");
            }
            var existNo = await Repository.Exist<BaseGoods>(x => x.GoodsNo == data.GoodsNo && x.GoodsName == data.GoodsName && x.Supplier == data.Supplier && x.GoodsId != data.GoodsId);
            if (existNo)
            {
                throw new BusinessException("保存失败,当前存在相同的原材料名称、SAP料号和供应商");
            }
        ...
        public List<KeyValueModel> GetExportField()
        {
            return new List<KeyValueModel>
            {
                ...
                new KeyValueModel {Key="GoodsName",Value="原材料名称",Remark=1},
                new KeyValueModel {Key="TypeName",Value="原材料分类", Remark = 3},
                new KeyValueModel {Key="OptionName",Value="原材料类型",Remark=4},
                new KeyValueModel {Key="GoodsModel",Value="原材料型号", Remark = 5},
                ...
                new KeyValueModel {Key="Photo",Value="原材料图片", Remark = 28},
        ...
        public async Task<string> ExportRawMaterial(string searchKey, string orderField, string orderType, int goodsClassifyId, List<KeyValueModel> fields)
        {
            if (fields.Count > 0)
            {
                ...
                string sql = $@"select  {fieldStr}
                            ...
                            where g.IsDeleted=0 and ( case when {goodsClassifyId}=0 then t.Group='RawMaterial' else g.GoodsClassifyId={goodsClassifyId} end ) 
                ...
                var fileName = $"原材料信息导出{DateTime.Now.ToStringNoSignExtension()}.xlsx";
