...
{
    public class ConsumablesMgr : DataPermissionHandler
    {
        ...
        public ConsumablesMgr(Repository repository, IFileStorage fileStorage, IMapper mapper, IConfiguration configuration) : base(repository)
        {
        ...
        {
            var typeData = await Repository.ClientDb.Queryable<BaseType>().Where(w => w.Group == BaseTypeGroup.Consumables.ToString()).ToListAsync();
        ...
        public async Task AddConsumablesType(BaseTypeDto data)
        {
            ...
            if (existModel)
            {
                throw new BusinessException("保存失败,当前耗材类型编码已存在");
            }
            var model = new BaseType
            {
                ...
                Remark = data.Remark,
                Group = BaseTypeGroup.Consumables.ToString()
            };
        ...
        public async Task UpdateConsumablesType(BaseTypeDto data)
        {
            ...
            if (!existModel)
            {
                throw new BusinessException("保存失败,当前耗材类型ID不存在或已删除");
            }
            var model = new BaseType
            {
                TypeId = data.TypeId,
        ...
        /// 删除耗材类型
        /// </summary>
        /// <param name="ConsumablesTypeId"></param>
        /// <returns></returns>
        public async Task DelConsumablesType(int ConsumablesTypeId)
        {
            await Repository.DeleteAsync<BaseType>(x => x.TypeId == ConsumablesTypeId);
        }
        ...
        public async Task<TableModel<ConsumablesDto>> GetConsumablesList(string userId, int pgSize, int pgIndex, string orderFiled, string orderType, int goodsClassifyId, string searchKey)
        {
            ...
            var data = Repository.ClientDb.Queryable<BaseGoods>() 
                  .LeftJoin<SysArgsOptions>((g, a)=>a.OptionId==g.GoodsTypeId && a.ArgsKey== BusinessConst.ConsumablesType)
                  .LeftJoin<BaseType>((g,  a,a2) => a2.TypeId == g.GoodsClassifyId && a2.Group ==BaseTypeGroup.Consumables.ToString())
                  .Where((g, a, a2)=>g.IsDeleted==false)
                  .WhereIF(goodsClassifyId > 0, (g, a, a2) => g.GoodsClassifyId == goodsClassifyId)
                  .WhereIF(goodsClassifyId == 0, (g,a, a2) => a2.Group == BaseTypeGroup.Consumables.ToString())
                  ...
                  .Select((g, a, a2) => new ConsumablesDto
                  {
                      GoodsId = g.GoodsId,
                      GoodsNo = g.GoodsNo,
                      ...
                      WarrantyPeriodDate = g.WarrantyPeriodDate,
            ...
            var res = new TableModel<ConsumablesDto>() { Total = total, Rows = data };
        ...
        public async Task<ConsumablesDto> GetConsumablesDetail(string spareId)
        {
            var data = await Repository.ClientDb.Queryable<BaseGoods>()
                ...
                .Where((g, a, a2, wbs) => g.GoodsId == spareId)
                .Select((g, a, a2, wbs) => new ConsumablesDto
                {
                    GoodsId = g.GoodsId,
                    GoodsNo = g.GoodsNo,
                    GoodsName = g.GoodsName,
        ...
        public async Task AddConsumables(ConsumablesDto data)
        {
            ...
            if (!string.IsNullOrEmpty(data.GoodsNo))
            {
                ...
                if (exist)
                {
                    throw new BusinessException("保存失败,当前存在相同的耗材名称和型号");
                }
                data.IsInSAP = true;
            } 
        ...
        public async Task UpdateConsumables(ConsumablesDto data)
        {
            ...
            if (!existId)
            {
                throw new BusinessException("保存失败,当前耗材不存在或已删除");
            }
        ...
        /// <returns></returns>
        public async Task DelConsumables(string[] sparePartId)
        {
            var models = await Repository.ClientDb.Queryable<BaseGoods>().Where(x => sparePartId.Contains(x.GoodsId)).ToListAsync();
        ...
        public List<KeyValueModel> GetExportField()
        {
            return new List<KeyValueModel>
            {
                ...
                new KeyValueModel {Key="GoodsName",Value="耗材名称",Remark=1},
                new KeyValueModel {Key="TypeName",Value="耗材分类", Remark = 3},
                new KeyValueModel {Key="OptionName",Value="耗材类型",Remark=4},
                new KeyValueModel {Key="GoodsModel",Value="耗材型号", Remark = 5},
                ...
                new KeyValueModel {Key="Photo",Value="耗材图片", Remark = 28},
        ...
        public async Task<string> ExportConsumables(string searchKey, string orderField, string orderType, int goodsClassifyId, List<KeyValueModel> fields)
        {
            if (fields.Count > 0)
            {
                ...
                string sql = $@"select  {fieldStr}
                            ...
                            where g.IsDeleted=0 and ( case when {goodsClassifyId}=0 then t.Group='Consumables' else g.GoodsClassifyId={goodsClassifyId} end ) 
                ...
                var fileName = $"耗材信息导出{DateTime.Now.ToStringNoSignExtension()}.xlsx";
